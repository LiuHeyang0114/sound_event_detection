...
import losses
import random
from augment import spec_aug, mixup, time_shifting, time_shifting_batch
...
class Runner(object):
    ...
    @staticmethod
    def _forward(model, batch, augment: Optional[Dict] = None):
        aids, feats, targets = batch
        if augment is not None:
            modes = augment['mode']
            if 'spec_aug' in modes:
                feats, targets = spec_aug(batch)
            if 'mixup' in modes:
                feats, targets = mixup(batch)
            if 'time_shifting' in modes:
                feats, targets = time_shifting(batch)
            if 'time_shifting_batch' in modes:
                feats, targets = time_shifting_batch(batch)
    ...
    @staticmethod
    def _forward_noaug(model, batch):
        aids, feats, targets = batch
        feats = feats.to(DEVICE).float()
        targets = targets.to(DEVICE).float()
        output = model(feats)
    ...
    def train(self, config_file, **kwargs):
        ...
        print(trainloader.dataset.datadim)
        ...
        for epoch in range(1, config['epochs'] + 1):
            augment = config['augment'] if 'augment' in config else None
            model.train()
            loss_history = []
            with torch.enable_grad(), tqdm(total=len(trainloader), unit="batch", leave=False) as pbar:
                for batch in trainloader:
                    optimizer.zero_grad()
                    output = self._forward(model, batch, augment)
                    loss = loss_fn(output)
                    loss.backward()
                    optimizer.step()
            ...
            with torch.no_grad(), tqdm(total=len(trainloader), unit="batch", leave=False) as pbar:
                for batch in cvdataloader:
                    output = self._forward_noaug(model, batch)
                    loss = loss_fn(output)
                    loss_history.append(loss.item())
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False):
                ...
                filtered_pred = utils.median_filter(
                    time_prob_batch, window_size=window_size, threshold=threshold)
